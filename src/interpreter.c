...
int interpret_funge(InstructionPointer *ip_ptr, char **funge)
{
    ...
    int iter, rand;
    int instruction_iter_count;
    char c; /* when we enter string mode */
    
    __debug("Processing %c\n", instruction);
    

    /* Execute the current iteration n times. Mostly its only 1, unless k instruction is reached */
    for (instruction_iter_count = 0;
         instruction_iter_count < ip_ptr->iteration_count;
         instruction_iter_count++)
    {
        switch(instruction) {
            /* 
    ...
    ip_ptr->iteration_count = 1; /* Reset the iter count */
